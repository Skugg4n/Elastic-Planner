...
### Fixed
- **Block kan nu verkligen dras inom sin egen area:** Förra fixen (v1.24.1) fixade bara `handleDragOver`. Det egentliga problemet var att `isOccupied`-checken i slot-renderingen inkluderade det dragna blocket, vilket gjorde att inga drop-targets renderades under blockets area. Nu exkluderas det dragna blocket och occuperade slots har `onDragOver` för korrekt drop-position.

## [1.24.1] - 2026-05-03

### Fixed
- **Block kan nu dras inom sin egen area:** Stora block gick inte att flytta 30 min eftersom drop-positionen låg "inuti" sig själv. `handleDragOver` exkluderar nu det dragna blocket vid hover-check så att man kan dra det till valfri tidpunkt, även inom dess nuvarande spann.

## [1.24.0] - 2026-04-24

### Added
- **Alt+drag duplicerar block:** Håll Alt/Option och dra ett block för att skapa en kopia. Originalet ligger kvar, kopian placeras där du släpper. Fungerar med auto-parallelize (dra kopian ovanpå ett annat block → de läggs bredvid varandra).
...
### Fixed
- **Dag-status-knapp (Normal/Halvdag/Ledig) var osynlig i normalläge:** Knappen för att cykla dagstatus renderade en tom sträng när status var "normal", vilket gjorde den oklickbar. Nu visas en diskret prick (`·`) som platshållare så att knappen alltid går att hitta och klicka. Detta är avgörande för att kunna undanta röda/halv-dagar från arbetsdags-räknaren (över/under 24h per vecka).
...
### Fixed
- **Kritisk bugg: Punkter/blixtar försvann vid sidomladdning.** Migrationsfunktionerna `migrateToProjectTracking`, `migrateBankRemoval` och `migrateLogsToPoints` skapade nya objekt utan att bevara `points`- och `excludedDays`-fält. Varje gång sidan laddades kördes migrationskedjan och nollställde all punkt-data. Fixat genom att använda `...weekData` spread i alla migrationssteg så att befintliga fält bevaras. Även `migrateParallelId` och `migrateCategoryColors` fick samma fix för framtidssäkring.
